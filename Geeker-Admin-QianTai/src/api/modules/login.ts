...
export const getUserInfoApi = () => {
  return http.get<Login.ResUserInfo>(PORT1 + `/user/info`, {}, { loading: false });
};

// 修改密码
export const changePasswordApi = (params: { oldPassword: string; newPassword: string }) => {
  return http.post(PORT1 + `/user/changePassword`, params, { loading: false });
};

// 用户退出登录
export const logoutApi = () => {
  return http.post(PORT1 + `/logout`, {}, { loading: false });
};
