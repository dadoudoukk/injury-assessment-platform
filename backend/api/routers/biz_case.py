...
async def case_list(
    ...
    if reportNumber and reportNumber.strip():
        stmt = stmt.where(CaseRecord.report_number.like(f"%{reportNumber.strip()}%"))
    if victimName and victimName.strip():
        stmt = stmt.where(CaseRecord.victim_name.like(f"%{victimName.strip()}%"))
...
async def case_create(
    ...
    db.add(
        CaseRecord(
            ...
            insurance_company_id=body.insuranceCompanyId,
...
async def case_update(
    ...
    if body.insuranceCompanyId is not None:
        row.insurance_company_id = body.insuranceCompanyId
